## CustomView
말 그대로 사용자가 자신의 편의, 사용 용도에 따라 정의한 뷰. 기본적으로 제공되는 뷰를 확장시켜 만든다.
몇가지 만드는 방식에 대해서 알아보도록 하겠다.
...
#### 2) Main.java 파일만으로 커스텀 뷰를 생성 할 수도 있다.
이를 응용해서 벽돌을 옮기는 게임을 만들 수도 있다. 커스텀 뷰를 생성하는 핵심 코드는 CustomView 클래스 이다.
...
```
activity_main.xml파일은 버튼 4개를 만들고, 연결만 했다. 전체 소스는 아래와 같다.
